...
object Profile:
  ...
  def result[N, P](population: Vector[Individual[P]], niche: Individual[P] => N, continuous: Vector[C], discrete: Vector[D], fitness: P => IArray[Double], keepAll: Boolean): Vector[Result[N, P]] =
  ...
  def adaptiveBreeding[P](lambda: Int, operatorExploration: Double, continuous: Vector[C], discrete: Vector[D], fitness: P => IArray[Double], reject: Option[Genome => Boolean]): Breeding[ProfileState, Individual[P], Genome] =
  ...
  def elitism[N, P](niche: Niche[Individual[P], N], mu: Int, components: Vector[C], discrete: Vector[D], fitness: P => IArray[Double]): Elitism[ProfileState, Individual[P]] =
  ...
  given isAlgorithm[N]: Algorithm[Profile[N], Individual[IArray[Double]], Genome, ProfileState] with {
  ...
  def result[N](profile: Profile[N], population: Vector[Individual[IArray[Double]]]): Vector[Result[N, IArray[Double]]] =
    result(population, profile.niche, profile.continuous, profile.discrete, identity, keepAll = false)
...
case class Profile[N](
  ...
  niche: Niche[CDGenome.DeterministicIndividual.Individual[IArray[Double]], N],
  nicheSize: Int = 20,
