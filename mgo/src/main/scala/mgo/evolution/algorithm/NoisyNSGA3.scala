...
object NoisyNSGA3 {
  ...
  def result[P: Manifest](population: Vector[Individual[P]], aggregation: Vector[P] => IArray[Double], continuous: Vector[C], discrete: Vector[D], keepAll: Boolean): Vector[Result[P]] = {
  ...
  def fitness[P: Manifest](aggregation: Vector[P] => IArray[Double]): Individual[P] => IArray[Double] =
  ...
  def elitism[S, P: Manifest](mu: Int, references: NSGA3Operations.ReferencePoints, historySize: Int, aggregation: Vector[P] => IArray[Double], components: Vector[C], discrete: Vector[D]): Elitism[S, Individual[P]] = {
  ...
  given isAlgorithm[P: Manifest]: Algorithm[NoisyNSGA3[P], Individual[P], Genome, NSGA3State] with
...
object NoisyNSGA3Operations:
  ...
  def aggregated[I, P](fitness: I => Vector[P], aggregation: Vector[P] => IArray[Double], accuracy: I => Double)(i: I): IArray[Double] =
    aggregation(fitness(i)) ++ IArray(1.0 / accuracy(i))
  ...
  def elitism[S, I](
    ...
    (s, population, candidates, rng) =>
      ...
      (s, NSGA3Operations.eliteWithReference[S, I](filtered, vectorFitness, references, mu)(rng))
