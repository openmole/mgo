...
class IntervalSet(listIntervals:Iterable[Interval]) {
     ...
     def mapBound (d: (Interval) => (String,(Double,Double))) = {listIntervals.map(x => d(x))}
  }
...
object IntervalSet {
  implicit def intervallDecorator()  = new {
    ...
    def generateBounds(listIntervals:IntervalSet)(implicit rng:Random):Map[String,(Double,Double)] = {
