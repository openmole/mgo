...
object crossover {
  ...
  def identityCrossover[G, S] = new Crossover[G, S] {
    override def apply(g1: G, g2: G): State[AlgorithmState[S], (G, G)] = State.state((g1, g2))
  }

  def blx[G, S](alpha: Double = 0.5)(implicit values: monocle.Lens[G, Seq[Double] @@ genome.Value]) = new Crossover[G, S] {
    def apply(g1: G, g2: G) =
      for {
        random <- random[S]
      } yield {
   ...
   *
   */
  def sbx[G, S](distributionIndex: Double = 2.0)(implicit values: monocle.Lens[G, Seq[Double] @@ genome.Value]) = new Crossover[G, S] {
  ...
  def dynamicCrossover[G, P, S](genomePart: monocle.Lens[G, Option[Int]], exploration: Double = 0.1)(ops: Crossover[G, S]*) =
    (pop: Population[Individual[G, P]]) => new Crossover[G, S] {
      def apply(g1: G, g2: G) =
        for {
