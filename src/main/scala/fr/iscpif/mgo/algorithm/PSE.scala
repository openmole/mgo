...
object pse extends niche.Imports {
  ...
  object PSE {
    ...
    implicit def isAlgorithm = new Algorithm[PSE, EvolutionState[HitMap, ?], Individual, Genome, EvolutionData[HitMap]] {

      def initialState(t: PSE, rng: Random) = EvolutionData[HitMap](random = rng, s = Map.empty)
      override def initialPopulation(t: PSE): EvolutionState[HitMap, Vector[Individual]] =
        deterministicInitialPopulation[EvolutionState[HitMap, ?], Genome, Individual](
          pse.initialGenomes(t.lambda, t.genomeSize), pse.expression(t.phenotype))

      def step(t: PSE) =
        deterministicStep[EvolutionState[HitMap, ?], Individual, Genome](
          pse.breeding(t.lambda, t.pattern, t.operatorExploration),
          pse.expression(t.phenotype),
          pse.elitism(t.pattern))

      def run[A](x: EvolutionState[HitMap, A], s: EvolutionData[HitMap]): (EvolutionData[HitMap], A) = mgo.unwrap(x, s)
    }
