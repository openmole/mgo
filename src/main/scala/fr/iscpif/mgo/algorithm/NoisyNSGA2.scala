...
//object NoisyNSGA2 {
//
//  def fitnessWithReplications[I](iHistory: Lens[I, Vector[Vector[Double]]])(i: I): Vector[Double] =
//    iHistory.get(i).transpose.map { vs => vs.sum / vs.size } ++ Vector(1.0 / iHistory.get(i).size.toDouble)
//
...
//        is: Vector[I] => is.map { iOperator }.collect { case Maybe.Just(op) => op }.groupBy(identity).mapValues(_.length.toDouble / parents.size).point[M]
//      }
