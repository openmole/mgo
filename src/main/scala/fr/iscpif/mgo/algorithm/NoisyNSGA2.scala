...
object noisynsga2 {
  ...
  object NoisyNSGA2 {
    ...
    implicit def isAlgorithm = new Algorithm[NoisyNSGA2, EvolutionState[Unit, ?], Individual, Genome, EvolutionData[Unit]] {
      def initialState(t: NoisyNSGA2, rng: Random) = EvolutionData[Unit](random = rng, s = ())

      def initialPopulation(t: NoisyNSGA2): EvolutionState[Unit, Vector[Individual]] =
      ...
      def step(t: NoisyNSGA2): Kleisli[EvolutionState[Unit, ?], Vector[Individual], Vector[Individual]] =
