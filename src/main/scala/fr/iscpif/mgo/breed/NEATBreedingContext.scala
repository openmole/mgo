...
import fr.iscpif.mgo.genome.NEATGenome.LinkInnovation
...
trait NEATBreedingContext extends BreedingContext with NEATGenome with NEATArchive {
  ...
  type StateBreedingState[Z] = State[BreedingState, Z]

  type BreedingContext[Z] = State[BreedingState, Z]
  //override implicit def monadBreedingContext: Monad[BreedingContext] = implicitly[Monad[StateBreedingState]]
