...
object TestNSGAII extends App {
  ...
  val nsgaII = new NSGAII {
    def lambda = 100
    def mu = 100
    override implicit def fitness: Fitness[Seq[Double]] = (i => Seq(i.phenotype))
  ...
  def randomGenome: State[Random, G] = State { rng: Random => (rng, Genome(GenomeValue(Seq.fill(dimensions)(rng.nextDouble)))) }
