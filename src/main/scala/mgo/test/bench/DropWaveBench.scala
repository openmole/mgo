...
object BenchDropWaveNSGAII extends App {
...
object BenchDropWaveAverageNSGAII extends App {
  ...
  val directory = File("/tmp/dropwave_avg/")
  ...
  def evolution[M[_]: Generation: Random: cats.Monad: StartTime: IO] = {
    val (nsga2, source) = sourceOf {
      def fitness(x: Vector[Double]) = {
        val samples = Vector.fill(100)(additiveGaussianNoise(0.25, dropWave.compute).apply(rng, x).head)
...
object BenchDropWaveNoisyNSGAII extends App {
  ...
  val directory = File("/tmp/dropwave_noisy/")
  directory.delete(swallowIOExceptions = true)
  directory.createDirectories()
  ...
  def evolution[M[_]: Generation: Random: cats.Monad: StartTime: IO] = {
    val (nsga2, source) = sourceOf {
      NoisyNSGA2[M](
        mu = 50,
        lambda = 50,
        fitness = additiveGaussianNoise(0.25, dropWave.compute),
        genomeSize = 2,
